...
Supports both keeping similar sequences and removing similar sequences
"""
...
def process_single_file(file_path: str, reference_dir: str, filter_mode: str, threshold_percentile: float, 
    ...
    try:
        ...
        # Calculate thresholds based on filter mode
        ...
        if filter_mode == 'keep_similar':
            # Keep similar sequences, remove outliers (high deviation)
            # Use high percentile threshold - remove sequences above this threshold
            unweighted_threshold = np.percentile(unweighted_scores, threshold_percentile) if unweighted_scores else float('inf')
            weighted_threshold = np.percentile(weighted_scores, threshold_percentile) if weighted_scores else float('inf')
        else:  # remove_similar
            # Remove similar sequences, keep different ones (low deviation)
            # Use low percentile threshold - remove sequences below this threshold
            similarity_percentile = 100 - threshold_percentile  # Convert to similarity threshold
            unweighted_threshold = np.percentile(unweighted_scores, similarity_percentile) if unweighted_scores else 0.0
            weighted_threshold = np.percentile(weighted_scores, similarity_percentile) if weighted_scores else 0.0
        
        # Filter sequences based on mode
        kept_records = []
        removed_sequences = []
        ...
        for score_info in deviation_scores:
            ...
            if filter_mode == 'keep_similar':
                # Remove sequences with high deviation (outliers/different from reference)
                is_removed = (unweighted_dev > unweighted_threshold or weighted_dev > weighted_threshold)
                removal_prefix = 'reference_outlier'
            else:  # remove_similar
                # Remove sequences with low deviation (similar to reference)
                is_removed = (unweighted_dev <= unweighted_threshold or weighted_dev <= weighted_threshold)
                removal_prefix = 'reference_similar'
            
            if is_removed:
                removal_reason = removal_prefix
                if filter_mode == 'keep_similar':
                    if unweighted_dev > unweighted_threshold and weighted_dev > weighted_threshold:
                        removal_reason = f'{removal_prefix}_both'
                    elif unweighted_dev > unweighted_threshold:
                        removal_reason = f'{removal_prefix}_unweighted'
                    elif weighted_dev > weighted_threshold:
                        removal_reason = f'{removal_prefix}_weighted'
                else:  # remove_similar
                    if unweighted_dev <= unweighted_threshold and weighted_dev <= weighted_threshold:
                        removal_reason = f'{removal_prefix}_both'
                    elif unweighted_dev <= unweighted_threshold:
...
def main():
    ...
    parser.add_argument('--filter-mode', required=True, choices=['keep_similar', 'remove_similar'],
                       help='Filter mode: keep_similar (remove outliers) or remove_similar (remove sequences similar to reference)')
    parser.add_argument('--threshold-percentile', type=float, default=90.0, 
                       help='Percentile threshold (keep_similar: remove above this percentile, remove_similar: remove below 100-percentile)')
    ...
    # Print configuration
    mode_description = {
        'keep_similar': 'keeping sequences similar to reference (removing outliers)',
        'remove_similar': 'removing sequences similar to reference (contamination filtering)'
    }
    
    print(f"Processing {len(input_files)} files with reference comparison")
    ...
    with ProcessPoolExecutor(max_workers=args.threads) as executor:
        future_to_file = {
            executor.submit(process_single_file, file_path, args.reference_dir, 
                          args.filter_mode, args.threshold_percentile, args.consensus_threshold, args.output_dir): file_path
    ...
    if args.filter_mode == 'keep_similar':
        print(f"Sequences removed (reference outliers): {total_removed}")
    else:
        print(f"Sequences removed (reference similar): {total_removed}")
